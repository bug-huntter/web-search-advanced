...
const st = {
	...
	testOk: {
		...
		fontSize: "12px",
		lineHeight: 1.6,
		color: "var(--dsw-alias-label-success, #2ea043)"
	},
	testWarn: {
		flex: 1,
		minWidth: 0,
		...
		fontSize: "12px",
		lineHeight: 1.6,
		color: "var(--dsw-alias-label-warning, #b88700)"
	},
	disabled: {
		opacity: .4,
		cursor: "default"
...
function WebSearchAdvancedSection(props) {
	...
	}), react.default.createElement("p", { style: st.hint }, t("card.maxUsesHint"))), react.default.createElement("div", { style: st.testRow }, react.default.createElement("button", {
		type: "button",
		style: disTest,
		disabled: state.testing || state.saving || !state.writable,
		onClick: () => void test()
	}, t("test.button")), state.testing ? react.default.createElement("p", { style: st.testResult }, t("test.testing")) : state.testResult === null ? react.default.createElement("p", { style: st.testResult }, t("test.hint")) : state.testResult.ok ? react.default.createElement("p", {
		style: st.testOk,
		role: "status"
	}, t("test.ok") + "：" + state.testResult.message) : state.testResult.canSave ? react.default.createElement("p", {
		style: st.testWarn,
		role: "status"
	}, t("test.warn") + "：" + state.testResult.message) : react.default.createElement("p", {
		style: st.failed,
		role: "status"
	}, t("test.blocked") + "：" + state.testResult.message)), react.default.createElement("div", { style: st.footer }, state.failed ? react.default.createElement("p", {
		style: st.failed,
		role: "status"
...
//#endregion
//#region src/client/testConnection.ts
function extractErrorMessage(body) {
	try {
		const p = JSON.parse(body);
		const d = typeof p.error === "string" ? p.error : p.error?.message ?? p.message;
		return d !== void 0 && d.length > 0 ? d : void 0;
	} catch {
		return body.length > 0 ? body.slice(0, 200) : void 0;
	}
}
function outcome(category, ok, status, message) {
	return {
		ok,
		canSave: category !== "config",
		status,
		category,
		message
	};
...
function apiKeyFormatHint(key) {
	...
	if (trimmed.length >= 2 && trimmed[0] === "\"" && trimmed[trimmed.length - 1] === "\"" || trimmed.includes("\"") || /\s/.test(key)) return "（提示：API Key 似乎误带了引号或空格——粘贴时带入多余字符是鉴权失败的常见原因）";
	return "";
}
/**
...
*/
async function testSearchConnection(values, timeoutMs = 2e4) {
	const base = values.baseURL.replace(/\/+$/, "");
	if (base.length === 0) return outcome("config", false, 0, "Base URL 为空");
	const model = values.model.trim();
	if (model.length === 0) return outcome("config", false, 0, "Model 为空");
	...
	const body = isCustom ? JSON.stringify({
		model,
		max_tokens: 4,
		messages: [{
			role: "user",
	...
	try {
		...
		return controller.signal.aborted ? outcome("transient", false, 0, `连接超时（超过 ${timeoutMs / 1e3} 秒）`) : outcome("unverifiable", false, 0, `浏览器无法直连（可能是跨域限制或网络问题）：${String(error)}。跨域限制只影响浏览器测试；保存后服务端实际搜索仍按其网络环境请求。`);
	} finally {
		clearTimeout(timeout);
	}
	const text = await response.text().catch(() => "");
	const status = response.status;
	if (response.ok) return outcome("ok", true, status, `连接成功（HTTP ${status}），模型 ${model} 可访问`);
	const message = extractErrorMessage(text) ?? `HTTP ${status}`;
	if (status === 429 || status >= 500) return outcome("transient", false, status, `${message}（临时限流/上游错误，配置本身通常有效）`);
	if (status === 401 || status === 403) {
		if (values.apiKey.length === 0) return outcome("unverifiable", false, status, `${message}（密钥由服务端解析，浏览器无法验证鉴权；保存后实际使用时会按服务端解析的密钥鉴权）`);
...
function buildState(scopeSnapshot, drafts, saving, failed, testing, testResult) {
...
function apply(ctx) {
	...
	ctx.inject(["settingsScope"], (scoped) => {
		...
		const save = async () => {
			...
			testResult = null;
			publish();
			const outcome$1 = await testSearchConnection(draftValues());
			testing = false;
			testResult = outcome$1;
			publish();
			if (!outcome$1.canSave) return;
			saving = true;
			failed = false;
			publish();
			try {
