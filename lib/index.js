		...
		return withRetry(async (innerSignal) => {
			...
			if (!response.ok) {
				...
				try {
					...
					if (innerSignal?.aborted === true || isAbortError(e)) throw searchAborted(innerSignal, e);
				}
				throw new SearchHttpError(message, {
...
var SearchHttpError = class extends WebError {
	status;
	retryAfterSeconds;
	constructor(message, options = {}) {
		super(message, "WEB_PROVIDER_ERROR", options.cause === void 0 ? void 0 : { cause: options.cause });
...
function isRetryableSearchError(error) {
	if (error instanceof SearchHttpError) return error.status !== void 0 && (error.status === 429 || error.status >= 500);
...
function retryDelayMs(error, attempt) {
	...
	if (typeof retryAfter === "number" && Number.isFinite(retryAfter) && retryAfter > 0) return Math.min(retryAfter * 1e3, RETRY_MAX_DELAY_MS);
	const base = RETRY_BASE_DELAY_MS * 2 ** Math.min(attempt, 6);
	return Math.min(base + Math.floor(Math.random() * base * .25), RETRY_MAX_DELAY_MS);
}
...
async function customHttpError(response, signal) {
	...
	return new SearchHttpError(message, {
		status: response.status,
		retryAfterSeconds: parseRetryAfterSeconds(response.headers.get("retry-after"))
	});
}
